...
export default function RestArguments({codeSnippets}) {
...
<p>Ballerina allows you to call functions with rest arguments, with an expression of a mapping or list type, spreading the members of the mapping or the list as individual arguments to the function.</p>


<p>If the type of the expression used in the rest argument is a list type, the rest argument is equivalent to specifying each member of the list value as a positional argument. If it is a mapping type, the rest argument is equivalent to specifying each field of the mapping value as a named argument, where the name and the value of the named argument come from the name and the value of the field. In either case, the static type of the expression must ensure that the equivalent positional and/or named arguments would be valid and that arguments are provided for all the required parameters.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.13.3/examples/rest-arguments", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run rest_arguments.bal`}</span>
<span>{`10`}</span>
<span>{`11`}</span>
<span>{`21`}</span>
<span>{`22`}</span>
...
<ul style={{ marginLeft: "0px" 
                        ...
                        <li>
                            <span>&#8226;&nbsp;</span>
                            <span><a href="/learn/by-example/aggregation/">Aggregation</a></span>
                        </li>
