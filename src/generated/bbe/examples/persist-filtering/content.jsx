...
export const codeSnippetData = [`import ballerina/persist as _;
...
export default function PersistFiltering({codeSnippets}) {
...
<p>The bal persist feature provide support to manage data persistence in a Ballerina package. It starts with defining the application’s data model. Once model is defined, the client API is generated with resources based on the model. The generated API can be used to query and manipulate the persistent data in the application.</p>

<p>The generated client API provides <code>get</code> resource method to retrieve all records from the data store. As the <code>get</code> resource method returns a stream of records, we can use query expressions to do additional filter. For more information, see <a href="/learn/by-example/query-expressions/">Query Expressions</a>.</p>


<blockquote>
<p><strong>Note:</strong> This example uses the Ballerina tables as the data store. For more information about other supported data stores, see <a href="/learn/supported-data-stores/">Supported Data Stores</a>.</p>
</blockquote>


<h4>Initialize the project</h4>

<p>Execute the command below to initialize <code>bal persist</code> in the project.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal persist init`}</span>
<span>{`
`}</span>
...
<p>Add <code>Employee</code> entity with the following fields in the <code>model.bal</code> file inside the <code>persist</code> directory.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      className="bg-transparent border-0 m-0 p-2 ms-auto"
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.13.3/examples/persist-filtering", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ bal persist generate --datastore inmemory --module store`}</span>
<span>{`
`}</span>
...
<blockquote>
<p><strong>Note:</strong> The <code>bal persist generate</code> command is a one-time generation task, and the generated client code is a part of the project. We can also integrate the client code generation with the project’s build process by executing the <code>bal persist add</code> command. This command will add the client code generation as a build task in the <code>Ballerina.toml</code> file. See <a href="learn/persist-cli-tool/">Persist CLI Commands</a> for more information.</p>
</blockquote>


<h4>Use the generated client API</h4>

<p>Using the generated client API, we can retrieve all records from the data store. The <code>get</code> resource method returns a stream of records. We can iterate through the stream and print the records.</p>
