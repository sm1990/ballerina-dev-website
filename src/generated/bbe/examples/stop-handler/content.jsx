...
export default function StopHandler({codeSnippets}) {
...
<p>A <code>StopHandler</code> is a function that is registered at runtime with a module and invoked during graceful shutdown.</p>

<p>This example demonstrates how to register a <code>StopHandler</code> that will be executed at the end of the program.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.13.3/examples/stop-handler", "_blank");
      }}
...
<p>Navigate to the directory that contains the ‘stop_handler.bal’ file, and run the ‘bal run’ command.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run stop_handler.bal`}</span>
...
<p>Invoke the service by executing the following cURL command in a new terminal to add a fruit item.</p>
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref2}>
      <code className="d-flex flex-column"><span>{`\$ curl http://localhost:9090/addToBasket -d 'Guava'`}</span>
