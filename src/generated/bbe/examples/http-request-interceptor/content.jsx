...
export default function HttpRequestInterceptor({codeSnippets}) {
...
<p>The <code>http:RequestInterceptor</code> is used to intercept the request and execute custom logic. A <code>RequestInterceptor</code> is a service object with only one resource method, which is executed before dispatching the request to the actual resource in the target service. This resource method can have parameters just like a usual resource method in an <code>http:Service</code>.</p>


<p>A <code>RequestInterceptor</code> can be created from a service class, which includes the <code>http:RequestInterceptor</code> service type. Then, this service object can be engaged at the service level by declaring an <code>http:InterceptableService</code> object.</p>


<p>This accepts an interceptor service object or an array of interceptor service objects as an interceptor pipeline, and the interceptors are executed in the order in which they are placed in the pipeline. Use <code>RequestInterceptors</code> to execute common logic such as logging, header manipulation, state publishing, etc., for inbound requests.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.13.3/examples/http-request-interceptor", "_blank");
      }}
