...
export default function CdcAdvancedService({codeSnippets}) {
...
<p>The <code>cdc:Service</code> connects to a MySQL database using the <code>mysql:CdcListener</code>, allowing you to handle change data capture (CDC) events as typed records. The listener detects database changes and calls the relevant remote method (<code>onRead</code>, <code>onCreate</code>, <code>onUpdate</code>, or <code>onDelete</code>) in your service, passing the event data.</p>


<p>You can attach multiple <code>cdc:Service</code> instances to a single <code>mysql:CdcListener</code>. This lets you group related event handling logic into separate services, making your code easier to organize and maintain. Each service can focus on a specific set of tables or event types, improving readability and separation of concerns.</p>
...
<Row className="bbeCode mx-0 py-0 rounded 
  ...
  <Col className="d-flex align-items-start" sm={12}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.13.3/examples/cdc-advanced-service", "_blank");
      }}
...
<Row className="bbeOutput mx-0 py-0 rounded " 
  ...
  <Col sm={12}>
    <pre ref={ref1}>
      <code className="d-flex flex-column"><span>{`\$ bal run cdc_advanced_service.bal`}</span>
...
<blockquote>
<p><strong>Tip:</strong> To insert additional records for testing, run the <code>test_cdc_advance_listener.bal</code> file provided in the <a href="https://github.com/ballerina-platform/ballerina-distribution/tree/master/examples/cdc-prerequisite">Change Data Capture Ballerina By Example - Prerequisites and Test Data</a>.</p>
</blockquote>
