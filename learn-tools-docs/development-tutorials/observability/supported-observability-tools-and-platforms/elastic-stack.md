...
Elasticsearch and Kibana are provided as [Cloud services](https://www.elastic.co/cloud). Alternatively, Docker containers can be used to set up Elasticsearch and Kibana as well.
...
Access Kibana to visualize the logs at [http://localhost:5601](http://localhost:5601). Add an index named `ballerina` and click on `Discover` to visualize the logs.
